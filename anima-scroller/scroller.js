...
function getComputedStyle1(el, key) {
...
var Scroller = function(container, options) {
  ...
  if (data.length && data[0].constructor === Object) {
    data.forEach(function(row) {
      html += '<div class="' + self.options.itemClass + '" data-name="' + row.name + '" data-value="' + row.value + '">' + row.name + '</div>';
    });
  } else {
    data.forEach(function(val) {
      html += '<div class="' + self.options.itemClass + '" data-name="' + val + '" data-value="' + val + '">' + val + '</div>';
    });
  }
  ...
  self.__itemHeight = parseInt(getComputedStyle1(indicator, 'height'), 10);
...
var members = {
  ...
  __scrollingComplete : function() {
    ...
    if (self.__prevValue !== null && self.__prevValue !== self.value) {
      self.options.onSelect({value: self.value, name: self.name});
    }
  },
