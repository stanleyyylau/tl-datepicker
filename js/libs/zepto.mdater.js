/* mdater
   基于 zepto 的日期选择控件。
   来源: http://www.bujichong.com/m/146

   添加人: 文明霖, 添加日期: 2016-08-30

   用法：
   1. 添加文件
        添加 css    <link rel="stylesheet" href="../../css/zepto.mdater.css">
        添加 zepto  <script type="text/javascript" src="../../js/libs/zepto.min.js"></script>
        添加 mdater <script type="text/javascript" src="../../js/zepto.mdater.js"></script>
   2. js
        $('.date').mdater(); // 没有参数
		...
		由于使用的是 val() 方法，所以，请使用表单控件。

   补充: 
   只有 minDate 和 maxDate 两个配置项。
   需要其他功能的，需要改代码。如添加事件，日期格式等。
   需要添加事件的，请阅读 panel.delegates 中的代码。（搜索即可，只有一个结果）

   2016-08-31 增加回调函数
   	1. 选择日期时的回调函数，参数是（年，月，日），月的计算同 JS Date 对象，比如，1月，值是 0，2月，值是 1。下同。
		onSelect: function (year, month, date) {},
	2. 点击确定时的回调函数，参数是（年，月，日）
		onOk: function (year, month, date) {},
	3. 点击取消时的回调函数，无参数。
		onCancel: function () {},
	4. 更改日历显示的年份时的回调函数，参数是年份，改变显示后的年份。
		onYearChange: function (year) {},
	5. 更改日历显示的月份时的回调函数，参数是更改后显示的（年，月）。
...
(function($){
	...
	$.fn.mdater = function(config){
		...
		var defaults = {
			maxDate : null,
			minDate : new Date(1970, 0, 1),
			onSelect: undefined, // function (year, month, date) {},
			onOk: undefined, // function (year, month, date) {},
			onCancel: undefined, // function () {},
			onYearChange: undefined, // function (year) {},
			onMonthChange: undefined, // function (year, month) {},
		};
			...
			afterShowPanel : function (mask,panel) {
				...
				panel.delegates({
					'.change_month' : function(){
						...
						if (option.onMonthChange && typeof(option.onMonthChange) === 'function') {
							option.onMonthChange(_this.value.year, _this.value.month);
						}
					},
					'.change_year' : function(){
						var add = $(this).hasClass('md_next') ? 1 : -1;
						_this._changeYear(add);
						if (option.onYearChange && typeof(option.onYearChange) === 'function') {
							option.onYearChange(_this.value.year);
						}
					},
					'.out_left, .out_right' : {
					...
					'.md_datearea li' : function(){
						...
						if (option.onSelect && typeof(option.onSelect) === 'function') {
							option.onSelect(_this.value.year, _this.value.month, _this.value.date)
						}
					},
					'.md_ok' : function(){
						...
						if (option.onOk && typeof(option.onOk) === 'function') {
							option.onOk(_this.value.year, _this.value.month, _this.value.date);
						}
						_this._hidePanel();
					},
					'.md_cancel' : function(){
